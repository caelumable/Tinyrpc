...
void Socket::Bind(const std::unique_ptr<InetAddress> &addr) {
  struct sockaddr_in tmp_addr = addr->GetAddr();
  unsigned int value = 0x1;
  setsockopt(fd_,SOL_SOCKET,SO_REUSEADDR,(void *)&value,sizeof(value));
...
int Socket::Accept(const std::unique_ptr<InetAddress> &addr) {
  // for server socket
  int clnt_sockfd = -1;
...
void Socket::Connect(const std::unique_ptr<InetAddress> &addr) {
  // for client socket
  struct sockaddr_in tmp_addr = addr->GetAddr();
...
void Socket::Connect(const char *ip, uint16_t port) {
  std::unique_ptr<InetAddress> addr (new InetAddress(ip, port));
  Connect(addr);
}
